...
class Agent:
    def __init__(self, start_x, start_y, grid_world):
        ...
        # Restart timing
        self.restart_button_time = None
    
    def move_agent(self, direction): # Beweeg de agent in een bepaalde richting
        if direction == "left" and self.x > 0 and self.grid_world.maze[self.y][self.x - 1] == 0:
            self.x -= 1
        elif direction == "right" and self.x < self.grid_world.cols - 1 and self.grid_world.maze[self.y][self.x + 1] == 0:
            self.x += 1
        elif direction == "up" and self.y > 0 and self.grid_world.maze[self.y - 1][self.x] == 0:
            self.y -= 1
        elif direction == "down" and self.y < self.grid_world.rows - 1 and self.grid_world.maze[self.y + 1][self.x] == 0:
            self.y += 1
